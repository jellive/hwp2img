...
**방법 A — 파일에 대고 마우스 오른쪽 버튼 (권장)**

파일을 어디로도 끌고 갈 필요가 없다. 받은 .hwp 파일이 보이는 그 자리에서 바로 한다.

1. .hwp 파일에 마우스 **오른쪽 버튼**을 누른다.
2. **[프로그램에서 열기]** → **[한글 사진으로 바꾸기]**
   - 거기 없으면 **[더 많은 옵션 표시]** 를 누르면 그 안에 **[한글 사진으로 바꾸기]** 가 있다.

> 이 메뉴는 프로그램을 **한 번이라도 실행하면 저절로 생긴다.** 따로 설치할 게 없다.
> 프로그램을 다른 폴더로 옮기거나 새 버전으로 바꿔도 다음 실행 때 알아서 다시 맞춘다.
...
- [ ] 실제 hwp 문서 하나로 **아이콘 위 드래그앤드롭** 테스트 — 보안 팝업이 뜨는지 확인
      (`hwp2img/security.py` 가 직접 등록하므로 안 떠야 한다. pyhwpx 의 자동 등록은
      `pip` 실행에 의존해서 **얼린 exe 에서 항상 실패**한다 — 그래서 우리가 직접 한다)
- [ ] **우클릭 메뉴** 확인 — 위에서 한 번 실행했으므로 이제 등록돼 있어야 한다.
      .hwp 파일 우클릭 → **[프로그램에서 열기]** 안에 "한글 사진으로 바꾸기" 가 보이는가?
      (Win11 에서 안 보이면 **[더 많은 옵션 표시]** 안에 있는지도 확인 — 어느 쪽에 뜨는지 적어 둘 것)
- [ ] 그 메뉴로 **띄어쓰기 든 파일명**(예: `공문 최종 (수정).hwp`)을 변환 — 인용이 틀리면
      공백 든 파일만 조용히 실패한다
- [ ] .hwp 를 **더블클릭**하면 여전히 **한글이** 열리는가 (우리 프로그램이 아니라)
