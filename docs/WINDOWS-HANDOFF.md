...
TODO·스텁 0건, 테스트 전부 통과(Windows CI `154 passed` · Mac `132 passed, 7 skipped` —
2026-09-19 실측), 커버리지 92%(2026-08 수치, 그 뒤로 다시 안 쟀다. 나머지 8%는 구조적으로 Mac에서 못 도는
...
**Windows에서 `154 passed` 가 안 나오면 그 자리에서 멈춰라.** (Mac은 `132 passed, 7 skipped` —
그 7개가 Windows 전용이라 거기서는 전부 돈다. 2026-09-19 실측)
그 밑에서 뭘 더 하든 신뢰할 수 없다. 이 테스트들은
...
1. **코드를 고치기 전에 `pytest`를 돌려서 기준선을 잡아라** — Windows `154 passed` /
   Mac `132 passed, 7 skipped` (2026-09-19 실측).
...
# 추가분 — Mac 세션 실측 (2026-09-19)

Windows 머신이 없는 세션이라 **Task 9 는 하나도 못 닫았다.** 대신 Mac 에서 닫을 수 있는
것만 닫고, 문서에 남아 있던 낡은 숫자와 유령 빨간불을 정리했다.

## 상태 한 줄

**`pyproject.toml` 은 `0.3.0` 인데 태그가 없다.** 릴리즈에 나가 있는 최신은 **v0.2.2**
(2026-08-24)라, 지금 어머니가 받을 수 있는 것도 거기까지다. 즉 우클릭 메뉴는 **만들어졌지만
아직 어머니에게 가 있지 않다.** 미릴리즈 커밋 3개:

```
2026-08-25 ci(smoke): 얼린 exe 의 등록을 실제로 재고, exe 를 아티팩트로 올린다
2026-08-25 fix(test): test_dnd_windows 가 Tk root 를 테스트마다 새로 만들던 것을 모듈 1개로
2026-08-25 feat: 탐색기 우클릭 메뉴로 변환 — 파일을 끌고 갈 필요가 없다
```

**태그는 일부러 안 냈다 (Jell 결정, 2026-09-19).** 아래 9항목이 Windows 에서 끝나기 전에는
`git tag v0.3.0` 을 밀지 않는다 — 2026-08-18 결정("5개 끝나기 전엔 어머니 PC 에 올리지 마라")의
연장이다. 릴리즈를 만드는 것 자체가 그 exe 를 `Latest` 로 만든다.

## 정리한 것 ①: CI 빨간불은 유령이었다

`test` 워크플로가 2026-08-25 이후 **3초짜리 실패**로 빨갛게 남아 있었다. 테스트가 깨진 게
아니다 — `gh run view` 의 ANNOTATIONS:
...
**GitHub 결제 차단**이다. 그런데 같은 날 03:37 에 test 가 그걸로 죽고 **03:39 에 smoke 는
5분48초 돌아서 성공**했다 — 잠깐 걸렸다 풀린 창이었고, 그 뒤로 main 에 push 가 없어서
빨간불만 25일 남아 있었다. 2026-09-19 재실행 결과:

```
✓ pytest (windows-latest) in 56s   → 154 passed
✓ pytest (macos-latest)   in 25s   → 132 passed, 7 skipped
```

★**4초짜리 실패는 로그가 아니라 ANNOTATIONS 를 먼저 봐라.** 로그는 아예 안 남는다
(`--log-failed` 가 `log not found` 를 돌려준다).

## 정리한 것 ②: 테스트 수가 세 번 드리프트했다

문서 안에 56 → 95 → 108 이 다 살아 있었다. 실측(2026-09-19):

| 어디 | 결과 |
|---|---|
| Windows CI | **154 passed** (스킵 0) |
| macOS CI · Mac 로컬 | **132 passed, 7 skipped** (4.2초) |

스킵 7개는 `test_dnd_windows.py` 6 + `test_shell_menu_windows.py` 1 — 전부 Windows 전용이라
Windows 에서는 다 돈다. **지시문 3곳만 고쳤고(`:14`, `:39`, `:114`), 날짜 붙은 기록 안의
95 는 그대로 뒀다** — 그건 그때 사실이었다.

⚠️ 커버리지 92% 는 **2026-08 수치고 다시 안 쟀다.** 근거로 쓰지 마라.

## 정리한 것 ③: `.github/release-notes.md` 가 망가진 채였다

`release.yml:144-162` 는 이 파일을 **복사한 뒤 개발자용 항목을 맨 끝에 덧붙인다.** 그런데
`## 개발자용` 헤딩이 v0.3.0 섹션보다 **위**에 있어서, 태그를 냈으면 이렇게 나갔다:

```
## 개발자용                              ← 비어 있음
## v0.3.0 — 파일에 대고 오른쪽 버튼...    ← 어머니용 내용인데 개발자용 아래
- 빌드 형태: ...                         ← v0.3.0 섹션 밑에 붙는다
```

어머니는 위에서부터 읽는데 **이 릴리즈의 전부인 "드래그 안 해도 된다"가 개발자용 아래 묻혀
있었다.** 순서를 바로잡아 `## 개발자용` 을 마지막 헤딩이자 마지막 줄로 뒀다.
**태그를 내봐야만 드러나는 결함이라 지금까지 아무도 못 봤다.**

같이 고친 것: 받는 법에 **4번(아이콘을 한 번 더블클릭)** 을 넣었다.
`2026-08-25-easier-for-mother.md` 의 codex 지적 `C2`("첫 실행 자가등록은 순환 조건 —
등록하려면 먼저 옛 방식으로 한 번 실행해야 한다")가 노트에는 반영이 안 돼 있었다.
노트는 "한 번이라도 실행하면 저절로 생깁니다"라고만 적고 **어떻게 그 한 번을 실행하는지를
안 적었다.**

## smoke 재실행으로 확정된 것 (run 35425233062, 3분10초, 초록)

`workflow_dispatch` 로 돌려 **v0.3.0 커밋의 진짜 얼린 exe** 를 아티팩트로 받았다(68MB).
로그에서 실제로 확인된 값:

```
command = "D:\a\hwp2img\hwp2img\dist\hwp to img.exe" "%1"
Terminate orphan process: hwp2img / 한글 사진으로 바꾸기 / 한글사진으로바꾸기 / hwp to img
758 WARNING: Failed to collect submodules for 'pyhwpx' ... com_error (Library not registered)
```

- **`"%1"` 인용이 살아 있다.** 빠졌으면 공백 든 파일명만 조용히 실패했을 자리다
- **자가 치유가 마지막으로 띄운 exe 를 가리킨다** (`hwp to img.exe`)
- **이름 4조합이 전부 부트로더를 통과했다** — ASCII / 한글+공백 / 한글 / ASCII+공백
- **pyhwpx 경고는 v0.2.0 과 똑같다.** 새 위험이 아니라 이미 어머니 PC 에서 변환에 성공한
  것과 같은 빌드 형태다

★ 단 **한컴오피스가 없는 러너**라 COM 경계 바깥만 본 것이다. 아래가 여전히 전부 미검증이다.

## Windows 에서 닫아야 하는 것 — 통합 체크리스트 9항목

앞선 추가분들에 흩어져 있던 것을 한 곳에 모았다. **원본 설명은 각 절에 그대로 있다.**

### A. watchdog — 이 프로그램의 하드 제약 (5항목, 위 "1-2" 절)

- [ ] A1. 한글을 띄워 둔 상태에서 멈추는 hwp 를 변환 → **먼저 띄운 한글이 살아있나** (PID 직접 비교)
- [ ] A2. **실제 암호 걸린 hwp** → 30초 안에 안내창이 뜨나 *(Task 9 전체에서 아직 아무도 시도 못 한 유일한 케이스)*
- [ ] A3. 타임아웃 후 작업 관리자에서 `Hwp.exe` 가 실제로 사라지나 (좀비 없음)
- [ ] A4. onefile exe 에서 `freeze_support()` 가 자식 spawn 을 실제로 되게 하나
- [ ] A5. 페이지 많은 문서에 30초가 충분한가 (모자라면 `watchdog.py` 의 `DEFAULT_TIMEOUT_SECONDS`)

### B. v0.3.0 우클릭 메뉴 (4항목, 위 "v0.3.0" 절)

- [ ] B1. 메뉴가 **탐색기에 실제로 보이나** (레지스트리에 값이 있는 것과 메뉴에 뜨는 것은 다르다)
- [ ] B2. Win11 에서 **[프로그램에서 열기]** 인가 **[더 많은 옵션 표시]** 인가
      → ★확인한 쪽으로 `.github/release-notes.md` 의 문장을 좁힌다. **지금은 양쪽을 다 적어 뒀다**
- [ ] B3. **띄어쓰기 든 파일명**이 한 덩어리로 오나 (CI 는 레지스트리 문자열까지만 봤다)
- [ ] B4. .hwp **더블클릭이 여전히 한글로** 가나

### C. 그 다음에만

- [ ] C1. README 의 "최초 1회 배포 체크리스트" (`README.md:41-49`)
- [ ] C2. 창을 띄워 둔 채 **연속 2~3회 변환** 후 `Hwp.exe` 개수 (위 "드롭존" 절 4번)
- [ ] C3. A·B 전부 통과하면 `git tag v0.3.0 && git push origin v0.3.0` → Actions 가 Release 에 exe 를 붙인다
- [ ] C4. 어머니 PC 에서 실제로 한 번 변환 = **끝**

## 별건 — 안 고친 것

`.venv` 가 **Python 3.14.7** 이다(`.python-version` 은 `3.11`). Mac 로컬은 그걸로도 132개가
통과하지만 **검증된 조합은 3.11** 이니 Windows 에서는 3.11 을 쓸 것. 2026-08-24 추가분이
적어 둔 드리프트가 그대로다 — 이번에도 안 고쳤다(Mac 쪽 편의 환경이라 급하지 않다).
