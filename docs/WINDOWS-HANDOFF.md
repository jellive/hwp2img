...
- 테스트 **79개 통과** (기존 56 + 신규 23). 신규 테스트는 변이 검사 8종을 전부 잡는다
  (책임 코드를 부수면 해당 테스트가 실제로 FAIL 한다 — 통과만으로는 증거가 아니라서 확인했다)
- 판정·상태 전이·경로 추출은 전부 Mac 에서 검증됨

## Windows 에서 **반드시** 확인해야 하는 것 (전부 미검증)

1. **더블클릭 → 창이 뜨는가.** `--onefile --noconsole` 빌드에서 tkinter 창이 뜨는지 한 번도
   안 봤다. 안 뜨면 PyInstaller 가 tcl/tk 를 수집했는지부터 본다(내장 훅이 하게 돼 있다).
