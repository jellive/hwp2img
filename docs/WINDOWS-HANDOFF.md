...
> 🟢 **실기기에서 뭘 해야 하는지만 알면 되면 → [`WINDOWS-CHECKLIST.md`](WINDOWS-CHECKLIST.md).**
> 준비물·빌드·13항목·통과 기준·실패 시 처치가 전부 거기 있고, 이 문서를 안 읽어도 돌아간다.
> **여기는 시간순 작업 기록이다** — "왜 이 항목이 있나" 가 궁금할 때 읽는 쪽.
...
## Windows 에서 닫아야 하는 것 → [`WINDOWS-CHECKLIST.md`](WINDOWS-CHECKLIST.md)

여기 있던 통합 체크리스트를 **실행 가능한 독립 문서로 옮겼다** (2026-09-19).
사본을 남기지 않는다 — 두 벌이 되면 표현이 갈려 드리프트한다.

**차단 9항목(A·B) + 후속 4항목(C).** 원본 설명(왜 그 항목이 있나)은 이 문서의
각 절에 그대로 있고, 체크리스트가 절 이름으로 되짚는다.

옮기면서 **코드와 대조해 고친 것 4가지** — 옛 목록을 그대로 따라가면 틀리는 자리다:

| 무엇이 틀렸나 | 어떻게 고쳤나 |
|---|---|
| **A3 의 통과 기준이 설계와 정반대였다.** "타임아웃 후 `Hwp.exe` 가 사라지나" 로 적혀 있었는데, 2026-08-18 결정으로 `_kill()` 은 **우리 파이썬 자식만** 죽이고 `Hwp.exe` 는 일부러 안 죽인다. 그대로 재면 **정상 동작을 실패로 기록**하고, 최악엔 금지된 PID kill 을 다시 넣게 된다 | 통과/실패가 아니라 **남은 개수를 적는 항목**으로. 쌓여서 문제가 될 때만 핸들 기반 종료를 넣는다고 명시 |
| **"9항목" 인데 체크박스가 13개**였다 | 차단 9(A·B) / 후속 4(C) 로 갈라서 명시 |
| **메뉴 자가등록의 순환 조건(B0)이 체크리스트에 없었다.** 릴리즈 노트에만 있었다 — 메뉴는 exe 를 한 번 실행해야 생기는데, 그 한 번을 어떻게 하는지가 빠져 있으면 B1 에서 막힌다 | `B0. 먼저 한 번 더블클릭` 을 선행 항목으로 |
| **C3 태그 푸시의 실패 복구 절차가 없었다.** `release.yml` 에는 `workflow_dispatch` 가 일부러 없어서(수동 실행이면 `GITHUB_REF_NAME` 이 브랜치라 버전 대조가 항상 죽는다) **태그를 지우고 다시 미는 것이 유일한 재시도 경로**다. 그리고 v0.2.1·v0.2.2 태그의 release 런이 실제로 결제 차단에 3~4초 만에 죽은 전례가 있다(run `32805891566`·`32805885222`) | 태그 삭제·재푸시 절차와 그 전례를 C3 안에 |

그리고 **2026-08-24 "드롭존 창" 절의 실기기 5항목 중 3개는 그 뒤 CI 가 덮었다**
(창 생성·진짜 Win32 드롭 메시지·변환 중 닫기). 통합 때 조용히 빠져 있던 것을
체크리스트의 "CI 가 이미 본 것" 표에 **덮은 근거와 함께** 적었다 — 누락과 닫힘을 구분할 수 있게.
